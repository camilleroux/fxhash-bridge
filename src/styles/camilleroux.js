...
export default class CamilleRouxStyle extends Style {
  ...
  beforeDraw () {
    ...
    for (let i = 0; i < 400; i++) {
      ...
      this._p5.noStroke()
      this._p5.ellipse(
        this._p5.random() * this._s,
        Math.abs(this._p5.randomGaussian(0, 0.3)) * this._s,
        0.001 * this._p5.random(1, 2) * this._s
      )
    }
    this._p5.pop()
  }
  ...
  drawTile ({ tilePoints, frontLeftCorner3DCoord, isBorder }) {
    ...
    for (let tilePointId = 0; tilePointId < tilePoints.length; tilePointId++) {
      const dotDensity = Math.pow(
        (this._gridSizeY - frontLeftCorner3DCoord.y) / this._gridSizeY,
        4
      )
      for (let i = 0; i < 4000 * dotDensity; i++) {
        const pointOriginVect = tilePoints[tilePointId].copy()
        const pointVect = pointOriginVect.lerp(
          tilePoints[(tilePointId + 1) % tilePoints.length],
          this._p5.random()
        )
        this._p5.rect(
          pointVect.x * this._s,
          (pointVect.y -
            Math.abs(
              this._p5.randomGaussian(0, isBorder ? 0.08 : 0.06) *
                this._p5.map(frontLeftCorner3DCoord.y, 0, this._gridSizeY, 1, 0)
            )) *
            this._s,
          this._s *
            this._p5.map(
              frontLeftCorner3DCoord.y,
              0,
              this._gridSizeY,
              0.0005,
              0.0003
