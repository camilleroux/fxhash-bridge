...
export default class Nobody extends Style {
  ...
  drawTile (tilePoints, frontLeftCorner3DCoord, isBorder) {
    ...
    this.noiseline(tilePoints[0].x*this._s, tilePoints[0].y* this._s,tilePoints[1].x * this._s,tilePoints[1].y* this._s,this.mul,this._p5.color(this.colorL[this.colorpicker]),this.fillcel,this._s,this.numberFunction,this.numberFunction==1?0.3:1)
    this.noiseline(tilePoints[3].x*this._s, tilePoints[3].y* this._s,tilePoints[0].x * this._s,tilePoints[0].y* this._s,this.mul,this._p5.color(this.colorL[this.colorpicker]),this.fillcel,this._s,this.numberFunction,this.numberFunction==1?0.3:1)
    this.noiseline(tilePoints[1].x*this._s, tilePoints[1].y* this._s,tilePoints[2].x * this._s,tilePoints[2].y* this._s,this.mul,this._p5.color(this.colorL[this.colorpicker]),this.fillcel,this._s,this.numberFunction,this.numberFunction==1?0.3:1)
    this.noiseline(tilePoints[3].x*this._s, tilePoints[3].y* this._s,tilePoints[2].x * this._s,tilePoints[2].y* this._s,this.mul,this._p5.color(this.colorL[this.colorpicker]),this.fillcel,this._s,this.numberFunction,this.numberFunction==1?0.3:1)
  ...
  endOfRoad(chooseEnd){
    ...
    switch(chooseEnd){
        ...
        case 2:
            return this.circleNoise(x,y-raio/4,raio/2*0.7,this.numberFunction==1?5:10,5,this._p5.color(this.colorL[this.colorpicker]),false,true,this.numberFunction)
            break;
        case 3:
            return this.circleNoise(x,y-raio/4,raio/2*0.7,this.numberFunction==1?5:10,5,this._p5.color(this.colorL[this.colorpicker]),false,false,this.numberFunction)
            break;
    }
  }
  ...
  circleNoise(x,y,sz,grant,dist,c1,fill,horizon,mul){
    let diam
    let countStage=0
    if(horizon){
    diam=y+sz
        for(let contFilt=y-sz;contFilt<diam;contFilt+=dist){
          let sg = (countStage*dist) + dist;
          let linelenght = (Math.sqrt((2*sg*sz) - (sg*sg))*2);
          let x1=x-sz/10-linelenght/2
          let x2=x+sz/10+linelenght/2
          this.noiseline(x1+this._p5.noise(contFilt),contFilt,x2+this._p5.noise(contFilt),contFilt,grant,c1,fill,this._s,mul)
          countStage++
        }
    }else{
    diam=x+sz
      for(let i=x-sz;i<diam;i+=dist){
          ...
          this.noiseline(i,y1+noisedislocate,i,y2+noisedislocate,grant,c1,fill,this._s,mul,2)
          countStage++
        }
    }
    }
...
angleBtwPoint=(p1,p2)=>{
  ...
  static name () { return 'Anaesthesia style Bridge' }
}